        ...
        public function calculate($orderid = 0, $update = true, $order_agentid = null)
        {
          ...
          g.commission3_rate,g.commission3_pay,g.commission,og.commissions,og.seckill,og.seckill_taskid,og.seckill_timeid from " . tablename('ewei_shop_order_goods') . '  og '
            ...
            if ($set['level'] > 0) {
				...
				$bounsdata =array(
					"orderid" => $orderid,
					"buymemberid" => $member['id'],
					"addtime" => time(),							
					"status" => 0,							
					"type" => 1,							
				);
				
				$levels = pdo_fetchall("select * from " . tablename("ewei_shop_commission_level") . " where uniacid=:uniacid order by id asc", array( ":uniacid" => $_W["uniacid"] ));
				...
				if(!empty($agentid)){
					...
					while ($fxagentid > 0 ){
						...
						$bounsdata['ceng'] = $ceng1;
						$bounsdata['memberid'] = $agentuser['id'];						
						if ($agentuser["isagent"] == 1 && $agentuser["status"] == 1 && $levelid  > 0){						
							
							if($levelid == 1){							
								if(!$hasl1){
									$bounsdata['type'] = 1;									
									$bounsdata['mcommission'] = $levels[0]['tmcommission'];
									$hasl1 = true;
									pdo_insert("ewei_shop_agentbouns", $bounsdata);
								}else{
									$bounsdata['mcommission'] = $levels[0]['pjcommission'];//平级奖
									$bounsdata['type'] = 2;
									if($levels[0]['pjcommission']>0){
										pdo_insert("ewei_shop_agentbouns", $bounsdata);
									}									
								}									
							}elseif($levelid == 2){
								if(!$hasl2){
									$bounsdata['type'] = 1;
									if($hasl1){
										$bounsdata['mcommission'] = $levels[1]['tmcommission']-$levels[0]['tmcommission'];
									}else{
										$bounsdata['mcommission'] = $levels[1]['tmcommission'];
									}
									$hasl2 = true;
									pdo_insert("ewei_shop_agentbouns", $bounsdata);										
								}else{
									$bounsdata['mcommission'] = $levels[1]['pjcommission'];//平级奖
									$bounsdata['type'] = 2;
									if($levels[1]['pjcommission']>0){
										pdo_insert("ewei_shop_agentbouns", $bounsdata);
									}
								}
							}elseif($levelid == 3){
								if(!$hasl3){
									$bounsdata['type'] = 1;
									if($hasl2){
										$bounsdata['mcommission'] = $levels[2]['tmcommission']-$levels[1]['tmcommission'];
									}else{
										if($hasl1){
											$bounsdata['mcommission'] = $levels[2]['tmcommission']-$levels[0]['tmcommission'];
										}else{
											$bounsdata['mcommission'] = $levels[2]['tmcommission'];
										}
									}
									$hasl3 = true;
									pdo_insert("ewei_shop_agentbouns", $bounsdata);
								}else{
									$bounsdata['mcommission'] = $levels[2]['pjcommission'];//平级奖
									$bounsdata['type'] = 2;
									if($levels[2]['pjcommission']>0){
										pdo_insert("ewei_shop_agentbouns", $bounsdata);
									}
								}
							}elseif($levelid == 4){
								if(!$hasl4){
									$bounsdata['type'] = 1;
									if($hasl3){
										$bounsdata['mcommission'] = $levels[3]['tmcommission']-$levels[2]['tmcommission'];
									}else{
										if($hasl2){
											$bounsdata['mcommission'] = $levels[3]['tmcommission']-$levels[1]['tmcommission'];
										}else{
											if($hasl1){
												$bounsdata['mcommission'] = $levels[3]['tmcommission']-$levels[0]['tmcommission'];
											}else{
												$bounsdata['mcommission'] = $levels[3]['tmcommission'];
											}
										}
									}
									$hasl4 = true;
									...
									$bounsdata['mcommission'] = $levels[3]['pjcommission'];//平级奖
									$bounsdata['type'] = 2;
									if($levels[3]['pjcommission']>0){
										pdo_insert("ewei_shop_agentbouns", $bounsdata);
									}
								}	
							}																																																	
...
//                print_r($where_time);exit;
			...
			if( in_array("total", $options) ) 
			{
				$agent_bouns = pdo_fetch("select sum(mcommission) as mcommission  from " . tablename("ewei_shop_agentbouns")  . " where status>=0 and memberid=".$member['id']);

				$system_total += $agent_bouns['mcommission'];
			}
			if( in_array("ok", $options) ) 
			{
				$agent_bouns = pdo_fetch("select sum(mcommission) as mcommission  from " . tablename("ewei_shop_agentbouns")  . " where status=1 and memberid=".$member['id']);

				$system_ok += $agent_bouns['mcommission'];
			}
			
			if( in_array("apply", $options) ) 
			{
				$agent_bouns = pdo_fetch("select sum(mcommission) as mcommission  from " . tablename("ewei_shop_agentbouns")  . " where status=2 and memberid=".$member['id']);

				$system_apply += $agent_bouns['mcommission'];
			}
			if( in_array("check", $options) ) 
			{
				$agent_bouns = pdo_fetch("select sum(mcommission) as mcommission  from " . tablename("ewei_shop_agentbouns")  . " where status=3 and memberid=".$member['id']);

				$system_check += $agent_bouns['mcommission'];
			}
			if( in_array("pay", $options) ) 
			{
				$agent_bouns = pdo_fetch("select sum(mcommission) as mcommission  from " . tablename("ewei_shop_agentbouns")  . " where status=4 and memberid=".$member['id']);

				$system_pay += $agent_bouns['mcommission'];
			}
			if( in_array("wait", $options) ) 
			{
				$agent_bouns = pdo_fetch("select sum(mcommission) as mcommission  from " . tablename("ewei_shop_agentbouns")  . " where status=0 and memberid=".$member['id']);

				$system_wait += $agent_bouns['mcommission'];
			}
			if( in_array("fail", $options) ) 
			{
				$agent_bouns = pdo_fetch("select sum(mcommission) as mcommission  from " . tablename("ewei_shop_agentbouns")  . " where status=-1 and memberid=".$member['id']);

				$system_fail += $agent_bouns['mcommission'];
			}
			
			$member["system_total"] = round($system_total, 2);
			$member["system_ok"] = round($system_ok, 2);
			$member["system_apply"] = round($system_apply, 2);
			$member["system_check"] = round($system_check, 2);
			$member["system_pay"] = round($system_pay, 2);
			$member["system_wait"] = round($system_wait, 2);
			$member["system_fail"] = round($system_fail, 2);
		...
		public function checkMemberTm($agentid = "0") 
		{
			global $_W;
			global $_GPC;
			if( empty($agentid) ) 
			...
			while ($fxagentid > 0){
				...
				$agentuser = m("member")->getMember($fxagentid);				
				$tmcount = $agentuser["tmcount"]+1;
				pdo_update("ewei_shop_member", array( "tmcount" => $tmcount ), array( "id" => $agentuser["id"] ));
        ...
        function upgradeLevelByAgent($openid)
        {
			...
			if ( $leveltype == 11 ){
				...
				while ($fxagentid > 0){
					$agentuser = m("member")->getMember($fxagentid);
					if ($agentuser["isagent"] == 1 && $agentuser["status"] == 1 ){
						$levelid = intval($agentuser["agentlevel"]);
						$downcount = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . ' where agentid=:agentid and status=1 and isagent=1 and uniacid=:uniacid ', array(':agentid' => $agentuser['id'], ':uniacid' => $_W['uniacid']));						
						$tmcount = 	$agentuser["tmcount"];			
						$newlevel = pdo_fetch("select id from " . tablename("ewei_shop_commission_level") . " where uniacid=:uniacid  and " . $downcount . " >= downcount and " . $tmcount . " >= tmdowncount  order by id desc limit 1", array( ":uniacid" => $_W["uniacid"] ));
						//升级
						if( !empty($newlevel) ) {
							if ($newlevel['id'] > $levelid){  
            ...
            if ($message_type == TM_COMMISSION_AGENT_NEW && empty($usernotice['commission_agent_new'])) {
                ...
                if ($tm['is_advanced']) {
                    ...
                    $message = array(
                        ...
                        'keyword2' => array('title' => '处理进度', 'value' => '新增下级通知', "color" => "#000000"),
                        'keyword3' => array('title' => '处理内容', 'value' => '您新增了一个下级' . $data['nickname'], "color" => "#000000"),
                        'remark' => array('value' => "\n感谢您的支持", "color" => "#000000")
                    );
                    $toopenid = $openid;
                ...
                if ($tm['is_advanced']) {
                    ...
                    $message = array(
                        ...
                        'keyword1' => array('title' => '业务类型', 'value' => '会员通知', "color" => "#000000"),
                        'keyword2' => array('title' => '处理进度', 'value' => $set['texts']['down'].'付款通知', "color" => "#000000"),
                        'keyword3' => array('title' => '处理内容', 'value' => '您的下级' . $data['nickname'] . '已经成功付款', "color" => "#000000"),
                        'remark' => array('value' => "\n感谢您的支持", "color" => "#000000")
                    );
                    $toopenid = $openid;
                    $datas[] = array('name' => '昵称', 'value' => $data['nickname']);
                ...
                if ($tm['is_advanced']) {
                    ...
                    $message = array(
                        ...
                        'keyword1' => array('title' => '业务类型', 'value' => '会员通知', "color" => "#000000"),
                        'keyword2' => array('title' => '处理进度', 'value' => $set['texts']['down'].'收货通知', "color" => "#000000"),
                ...
                if ($tm['is_advanced']) {
                    ...
                    $message = array(
                        ...
                        'keyword1' => array('title' => '业务类型', 'value' => '会员通知', "color" => "#000000"),
                        'keyword2' => array('title' => '处理进度', 'value' => '提现申请提交通知', "color" => "#000000"),
                        'keyword3' => array('title' => '处理内容', 'value' => '您的佣金提现申请已提交', "color" => "#000000"),
                        'remark' => array('value' => "\n感谢您的支持", "color" => "#000000")
                    );
                    $toopenid = $openid;
                    $datas[] = array('name' => '昵称', 'value' => $member['nickname']);
                ...
                if ($tm['is_advanced']) {
                    ...
                    $message = array(
                        ...
                        'keyword1' => array('title' => '业务类型', 'value' => '会员通知', "color" => "#000000"),
                        'keyword2' => array('title' => '处理进度', 'value' => '提现申请审核通知', "color" => "#000000"),
                        'keyword3' => array('title' => '处理内容', 'value' => '您的佣金提现申请审核完成', "color" => "#000000"),
                        'remark' => array('value' => "\n感谢您的支持", "color" => "#000000")
                    );
                    $toopenid = $openid;
                    $datas[] = array('name' => '昵称', 'value' => $member['nickname']);
                ...
                if ($tm['is_advanced']) {
                    ...
                    $message = array(
                        ...
                        'keyword1' => array('title' => '业务类型', 'value' => '会员通知', "color" => "#000000"),
                        'keyword2' => array('title' => '处理进度', 'value' => $set['texts']['commission1'] . '打款通知', "color" => "#000000"),
                ...
                if ($tm['is_advanced']) {
                    ...
                    $message = array(
                        ...
                        'keyword1' => array('title' => '业务类型', 'value' => '会员通知', "color" => "#000000"),
                        'keyword2' => array('title' => '处理进度', 'value' => $set['texts']['agent'] . '等级升级通知', "color" => "#000000"),
                        'keyword3' => array('title' => '处理内容', 'value' => '您的'.$set['texts']['agent'].'等级从' . $data['oldlevel']['levelname'] . '升级为' . $data['oldlevel']['levelname'] . '特此通知!', "color" => "#000000"),
                        'remark' => array('value' => "\n感谢您的支持", "color" => "#000000")
                    );
                    $toopenid = $openid;
                ...
                if ($tm['is_advanced']) {
                    ...
                    $message = array(
                        ...
                        'keyword1' => array('title' => '业务类型', 'value' => '会员通知', "color" => "#000000"),
                        'keyword2' => array('title' => '处理进度', 'value' => '成为' . $set['texts']['agent'] . '通知', "color" => "#000000"),
                        'keyword3' => array('title' => '处理内容', 'value' => '成为'.$set['texts']['agent'].'通知', "color" => "#000000"),
                        'remark' => array('value' => "\n感谢您的支持", "color" => "#000000")
                    );
                    $toopenid = $openid;
                    $datas[] = array('name' => '昵称', 'value' => $data['nickname']);
                ...
                if ($tm['is_advanced']) {
                    ...
                    $message = array(
                        ...
                        'keyword2' => array('title' => '处理进度', 'value' => '商户名称:' . $member['nickname'], "color" => "#000000"),
                        'keyword3' => array('title' => '处理内容', 'value' => '佣金提现:' . $data['commission'] . '元', "color" => "#000000"),
                        'remark' => array('value' => "\n感谢您的支持", "color" => "#000000")
                    );
                    $toopenid = $tm['openid'];
                    $datas[] = array('name' => '昵称', 'value' => $member['nickname']);
