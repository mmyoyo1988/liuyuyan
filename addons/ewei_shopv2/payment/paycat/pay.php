...
//    $istype = $_POST["istype"];
    ...
    $orderuid = "username";       //此处传入您网站用户的用户名，方便在平台后台查看是谁付的款，强烈建议加上。可忽略。

    //校验传入的表单，确保价格为正常价格（整数，1位小数，2位小数都可以），支付渠道只能是1或者2，orderuid长度不要超过33个中英文字。

    //此处就在您服务器生成新订单，并把创建的订单号传入到下面的orderid中。
    $goodsname = "一只小测试";
    $orderid = "OID_20190001";    //每次有任何参数变化，订单号就变一个吧。
    $uid = "5147";//"此处填写平台的uid";
    $token = "sRwW3R5KBkZrSqQewqurh5wFKaV5YKW5";//"此处填写平台的Token";
    $return_url = $_POST['url'] . 'addons/ewei_shopv2/payment/paycat/payreturn.php';
    $notify_url = $_POST['url'] . 'addons/ewei_shopv2/payment/paycat/paynotify.php';
