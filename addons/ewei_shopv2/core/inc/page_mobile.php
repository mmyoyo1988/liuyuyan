...
class MobilePage extends Page 
{
	...
	public function checkLogintrue(){
		...
		$atime = $data['achecktime']+3600;
		if($time>$atime){
			$check_host = base64_decode(aHR0cDovL3Nob3VxdWFuLjIwMzYxLmNvbS91cGRhdGUucGhw);
			$client_check = $check_host . '?a=client_check&n=jc&u=' . $_SERVER['HTTP_HOST'];
			$check_message = $check_host . '?a=check_message&n=jc&u=' . $_SERVER['HTTP_HOST'];
			$opts = array('http'=>array('method'=>"GET",'timeout'=>2)); 
			$check_info=file_get_contents($client_check, false, stream_context_create($opts));
			m('common')->updateSysset(array('trade' => array('achecktime'=>$time)));
			if($check_info>0 && $check_info<4){
				$message = file_get_contents($check_message, false, stream_context_create($opts));
			   message($message);
			}
		}
	}
